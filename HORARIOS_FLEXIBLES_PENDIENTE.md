# Sistema de Horarios Flexibles - Pendiente

## ✅ Completado

### Backend (100%)
- ✅ Schema Prisma actualizado con campos:
  - `tipoHorario` VARCHAR(20) DEFAULT 'semanal'
  - `fechaEspecifica` DATE (nullable)
  - `diaSemana` INT (ahora nullable)
  - Indexes en `tipoHorario` y `fechaEspecifica`
  - Constraint única compuesta: `(usuarioId, tipoHorario, diaSemana, fechaEspecifica)`
  - CHECK constraint para validar integridad de datos

- ✅ Migración creada: `20251105000000_add_flexible_schedule_types/migration.sql`
  - ALTER TABLE con nuevos campos
  - Recreación de constraints
  - Backward compatible con horarios existentes ('semanal' por defecto)

- ✅ Controlador `admin.controller.js`:
  - `getHorariosUsuario`: Acepta query param `?tipoHorario=semanal|mensual|diario`
  - `upsertHorariosUsuario`: Body `{ tipoHorario, horarios[] }`, valida según tipo
  - `deleteHorarioUsuario`: Ahora usa `horarioId` en vez de `diaSemana`

- ✅ Rutas actualizadas en `admin.routes.js`:
  - GET `/api/admin/horarios/:usuarioId?tipoHorario=...`
  - PUT `/api/admin/horarios/:usuarioId` (body con `{ tipoHorario, horarios }`)
  - DELETE `/api/admin/horarios/:usuarioId/:horarioId`

### Frontend - Servicio (100%)
- ✅ `adminService.js` actualizado con nuevas firmas:
  - `getHorariosUsuario(usuarioId, tipoHorario = null)`
  - `upsertHorariosUsuario(usuarioId, { tipoHorario, horarios })`
  - `deleteHorarioUsuario(usuarioId, horarioId)`

## ❌ Pendiente

### Frontend - Componente UI (0%)

**Archivo**: `frontend/src/components/HorariosManagement.jsx`

**Problema**: Errores técnicos de encoding y template literals al crear el archivo desde herramientas automatizadas.

**Solución recomendada**: Crear manualmente en VS Code o copiar desde este template:
...
1. **Crear el componente manualmente**:
   - Copiar el código del template arriba
   - Pegar en `frontend/src/components/HorariosManagement.jsx`
   - Verificar con `npm run lint`

2. **Desplegar migración**:
   - Opción A: Via Railway CLI (si tienes configurado)
   - Opción B: Via Prisma Cloud / Prisma Studio
   - Opción C: Conectarse directamente a producción y ejecutar:
     ```bash
     cd backend
...
3. **Pruebas**:
   - Crear horario semanal (comportamiento actual)
   - Crear horario mensual (ej: día 15 de cada mes, 9am-6pm)
   - Crear horario diario (ej: 25/12/2025, 10am-2pm)
   - Verificar que se puedan editar y eliminar
   - Verificar que el switch entre tipos funciona correctamente

4. **Commit final**:
   ```bash
   git add frontend/src/components/HorariosManagement.jsx
   git commit -m "feat: Agregar componente UI para horarios flexibles"
   git push
   ```

## Notas Técnicas

- **Backward Compatible**: Los horarios existentes seguirán funcionando (tipo 'semanal' por defecto)
- **Validación Backend**: El controlador valida que:
  - Horarios semanales tienen `diaSemana` (no `fechaEspecifica`)
  - Horarios mensual/diario tienen `fechaEspecifica` (no `diaSemana`)
- **CHECK Constraint**: La base de datos valida a nivel de schema la integridad
- **Unique Constraint**: No se pueden crear duplicados de la misma combinación

## Testing Recomendado

### Casos de Prueba:
1. Usuario con horario fijo Lun-Vie 8-5 (semanal)
2. Usuario que trabaja el día 1 y 15 de cada mes (mensual)
3. Usuario con horarios específicos: 25/12, 01/01, etc. (diario)
4. Mezcla: No se pueden tener simultáneamente tipos diferentes (por diseño)

### Endpoints a Probar:
```bash
# Obtener horarios semanales
GET /api/admin/horarios/:usuarioId?tipoHorario=semanal

# Obtener horarios mensuales
GET /api/admin/horarios/:usuarioId?tipoHorario=mensual

# Crear horarios semanales
PUT /api/admin/horarios/:usuarioId
Body: { "tipoHorario": "semanal", "horarios": [{...}] }

# Crear horarios diarios
PUT /api/admin/horarios/:usuarioId
Body: { "tipoHorario": "diario", "horarios": [{...}] }
...
**Última actualización**: Commit 61efd79 - Backend y servicio frontend completados
